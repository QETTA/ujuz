...
export function DataBlockCard({ title, content, confidence, source, className }: DataBlockCardProps) {
  ...
  return (
    <div className={cn('rounded-xl border border-border bg-surface-elevated p-3', className)}>
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-text-primary">{title}</h4>
        <span className="text-xs text-text-tertiary">신뢰도: {confidenceLabel}</span>
      </div>
      <p className="mt-1 text-xs text-text-secondary leading-relaxed">{content}</p>
      {source && <p className="mt-1 text-xs text-text-tertiary">출처: {source}</p>}
    </div>
  );
}
